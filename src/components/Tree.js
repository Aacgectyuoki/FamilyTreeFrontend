...
const Tree = () => {
  ...
  const handleGedcomUpload = async (file) => {
    const formData = new FormData();
    formData.append('gedcomFile', file);
  
    try {
      await axios.post('/api/family/upload', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      fetchFamilyMembers(); // Fetch family members after successful upload
    } catch (error) {
  ...
  return (
    <div className="tree-container">
      ...
      <UploadGedcom onUpload={handleGedcomUpload} />
      {members.length > 0 ? (
